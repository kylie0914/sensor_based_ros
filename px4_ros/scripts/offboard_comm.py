...
class OffboardCtrl:
    ...
    def set_prev_position(self):
        self.prev_position_x = self.local_position.pose.position.x
        self.prev_position_y = self.local_position.pose.position.y
        self.prev_position_z = self.local_position.pose.position.z	
    
    def drift_check(self):    	
        if abs(self.local_position.pose.position.x - self.prev_position_x)> self.drift_threshold:
            self.drift_x = self.local_position.pose.position.x - self.prev_position_x
            self.IsDrift = True 
            self.halt()                      
        if abs(self.local_position.pose.position.y - self.prev_position_y)> self.drift_threshold:
            self.drift_y = self.local_position.pose.position.y - self.prev_position_y
            self.IsDrift = True 
            self.halt()                   
        if abs(self.local_position.pose.position.z - self.prev_position_z)> self.drift_threshold:	
            self.drift_z = self.local_position.pose.position.z - self.prev_position_z
            self.IsDrift = True
            self.halt()
        ...
        return self.IsDrift, self.drift_x, self.drift_y, self.drift_z         
